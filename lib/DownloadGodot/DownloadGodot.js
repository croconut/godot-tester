...
const {
    ...
    deleteTmpDir,
    getTmpDir
} = require('./helpers/TmpDirHelpers/TmpDirHelpers');
...
function generateDownloadUrl() {
    ...
    if (customGodotDlUrl && customGodotDlUrl !== '') {
...
function findExecutablePath(unzippedFiles) {
    const unzipDir = getTmpDir();
    const userSpecifiedPath = getCliArgs().customGodotBinaryPath;

    const userSpecifiedExe = path.join(unzipDir, userSpecifiedPath)
    if(userSpecifiedPath !== '' && fs.existsSync(userSpecifiedExe)) {
        return {
            type: 'file', // file type
            mode: 33261, // executable mode
            path: userSpecifiedPath, // path to the file
        };
    }
