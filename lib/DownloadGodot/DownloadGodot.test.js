...
jest.mock('./helpers/TmpDirHelpers/TmpDirHelpers', () => ({
    getTmpDir: jest.fn(),
    createTmpDir: jest.fn(),
    deleteTmpDir: jest.fn(),
}));
jest.mock('./helpers/GenerateGodotFilename/GenerateGodotFilename');
jest.mock('./helpers/PerformDownload/PerformDownload');

const yargs = require('yargs');
const fs = require('fs');
const decompress = require('decompress');

const generateGodotFilename = require('./helpers/GenerateGodotFilename/GenerateGodotFilename');
const { getTmpDir } = require('./helpers/TmpDirHelpers/TmpDirHelpers');
const performDownload = require('./helpers/PerformDownload/PerformDownload');
...
describe('downloadGodot', () => {
    ...
    describe('given custom godot download url', () => {
        beforeEach(() => {
            ...
            yargs.argv.customGodotBinaryPath = 'output/godot-binary'
        });
        ...
        describe('given custom godot binary path', () => {
            it('should return the custom godot binary path if present', async () => {
                fs.existsSync.mockReturnValue(true);

                const executablePath = await downloadGodot();
                expect(executablePath).toEqual(`${fakeTempDir}/output/godot-binary`);
                expect(fs.existsSync).toHaveBeenCalledWith(`${fakeTempDir}/output/godot-binary`);
            });

            it('should search for the default godot binary path if not present', async () => {
                const executablePath = await downloadGodot();
